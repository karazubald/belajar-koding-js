...
function angkaPalindrome(num) {
    let isPalindrome = true; // asumsi awal adalah palindrom
    let palindrome_num = (num + 1);
    let num_string = palindrome_num.toString();
    let num_array = num_string.split('');
    let num_terbalik = '';

    for (let index = num_array.length-1; index >= 0; index--) {
        num_terbalik += num_array[index];
    }

    if (num_terbalik !== num_string) {
        isPalindrome = false;
    }
